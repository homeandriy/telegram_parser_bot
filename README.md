...
GET /api/app_ico повертає PNG-іконку застосунку як stream для завантаження. GET /api/health також містить абсолютний URL цього endpoint у полі pp_ico.
...
3. Передавайте `location_uid` у дії правила, а не у каналі. Для міста зі спеціальним статусом (наприклад, м. Київ — `31`) використовуйте його єдиний UID. Порожній UID на вході нормалізується до `31` і не зберігається порожнім.

`GET /api/rules` повертає кожному правилу поле `location`. Коли UID не передано під час створення чи копіювання правила, зберігається UID Києва `31`. Для UID району API автоматично повертає район і його батьківську область:
...
`POST /api/rules/copy` копіює правило на інший канал. Передайте `source_resource_id`, `target_resource_id`, за потреби `location_uid` та `match_terms`. Порожній `location_uid` зберігається як `31`; без `match_terms` умови збігу копіюються з джерела. Ідентична копія для цільового каналу повертає `409 Conflict`.
...
`deployment/.env` ігнорується Git. API перевіряється раз на 10 секунд. У правилі зберігається лише `location_uid` району з [таблиці UID](https://docs.google.com/spreadsheets/d/1XnTOzcPHd1LZUrarR1Fk43FUyl8Ae6a6M7pcwDRjNdA/edit?gid=0#gid=0); область визначається через зв’язок у БД. Якщо UID не вказано під час створення, зберігається Київ (`31`). Канал переходить на швидкий інтервал тільки при активній повітряній тривозі в UID хоча б одного його правила.
